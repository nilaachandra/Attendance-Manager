...
const AddBatches = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const batchData = {batchname: batchName, created_by_user_name: currentUser}
    ...
    <SignedIn>
      <div className="w-full min-h-[50vh] flex flex-col justify-center items-center gap-3">
        <form onSubmit={handleSubmit} className="w-1/3 flex flex-col justify-center items-center border border-black p-4 gap-3">
          <Input type="text" name="name" placeholder="Add Batch Name" value={batchName} onChange={handleInputChange} className='text-center'/>

          <Button type='submit' className='btnHover raleway-regular font-bold'>Add Batch</Button>
        </form>
      </div>
    </SignedIn>
  )
